...
public class MessageProducerServiceImpl implements MessageProducerService {
    ...
    @Value("${kafka.producer.parking.topic}")
    private String upParkingTopic;
     ...
     */
    public InterfaceResult sendMessage(String topic, Object data, Boolean ifPartition, Integer partitionNum, String role){
        InterfaceResult interfaceResult = new InterfaceResult();
        topic = topic.equals("-1")?upTopic:(topic.equals("-2")?upParkingTopic:topic);
...
//        String valueString = JSON.toJSONString(data);
        if(ifPartition){
            ...
            interfaceResult = checkProRecord(result);
...
//            Result res = checkProRecord(result);
    ...
    private InterfaceResult checkProRecord(ListenableFuture<SendResult<String, String>> res){
        InterfaceResult interfaceResult = new InterfaceResult();
        if(res!=null){
            try {
                ...
                if(offsetIndex!=null && offsetIndex>=0){
                    ...
                    interfaceResult.InterfaceResult600("未查到返回数据的offset");
                }
            } catch (InterruptedException e) {
                e.printStackTrace();
                interfaceResult.InterfaceResult600("发送消息超时");
            } catch (ExecutionException e) {
                e.printStackTrace();
                interfaceResult.InterfaceResult600("发送消息超时");
            }
        }else{
            interfaceResult.InterfaceResult600("未查询到返回结果");
        }
        return interfaceResult;
    }
}
